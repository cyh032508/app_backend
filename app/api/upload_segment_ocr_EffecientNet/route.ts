...
/**
 * @swagger
 * /api/upload_segment_ocr_EffecientNet:
 *   post:
 *     summary: 使用 EfficientNet 模型進行 OCR 識別
 *     description: 接收圖片檔案，先進行格線切割，再對切割後的每個格子圖片進行 OCR 辨識。注意：此端點需要 Python 服務支持 PyTorch EfficientNet 模型
 *     tags: [OCR]
 *     requestBody:
 *       required: true
 *       content:
